...
static size_t _ntoa_format(out_fct_type out, char* buffer, size_t idx, size_t maxlen, char* buf, size_t len,
  ...
  // pad leading zeros
  if (!(flags & FLAGS_LEFT)) {
    if (width && (flags & FLAGS_ZEROPAD) && (negative or (flags & (FLAGS_PLUS | FLAGS_SPACE)))) { width--; }
  ...
  // handle hash
  if (flags & FLAGS_HASH) {
    if (!(flags & FLAGS_PRECISION) && len && ((len == prec) or (len == width))) {
      len--;
      if (len && (base == 16U)) { len--; }
    }
...
static size_t _ftoa(out_fct_type out, char* buffer, size_t idx, size_t maxlen, double value, unsigned int prec,
  ...
  // buffers == bad
  if ((value > PRINTF_MAX_FLOAT) or (value < -PRINTF_MAX_FLOAT)) {
#if defined(PRINTF_SUPPORT_EXPONENTIAL)
  ...
  diff               = tmp - frac; // NOLINT

  if (diff > 0.5) {
    ++frac;
    // handle rollover, e.g. case 0.99 with prec 1 is 1.0
    if (frac >= pow10[prec]) { // NOLINT
      frac = 0;
      ++whole;
    }
  } else if (diff < 0.5) {
  } else if ((frac == 0U) or (frac & 1U)) {
  ...
  if (prec == 0U) {
    ...
    if ((!(diff < 0.5) or (diff > 0.5)) && (whole & 1)) {
  ...
  // pad leading zeros
  if (!(flags & FLAGS_LEFT) && (flags & FLAGS_ZEROPAD)) {
    if (width && (negative or (flags & (FLAGS_PLUS | FLAGS_SPACE)))) { width--; }
...
static size_t _etoa(out_fct_type out, char* buffer, size_t idx, size_t maxlen, double value, unsigned int prec,
  ...
  if ((value != value) or (value > DBL_MAX) or (value < -DBL_MAX)) {
...
static int _vsnprintf(out_fct_type out, char* buffer, const size_t maxlen, const char* format, va_list va) {
  ...
  while (*format) {
    ...
    case 'g':
    case 'G':
      if ((*format == 'g') or (*format == 'G')) { flags |= FLAGS_ADAPT_EXP; }
      if ((*format == 'E') or (*format == 'G')) { flags |= FLAGS_UPPERCASE; }
    ...
    case 's': {
      ...
      // string output
      while ((*p != 0) && (!(flags & FLAGS_PRECISION) or precision--)) {
