...
static int _vsnprintf_output(const char* str, size_t len, void* state) {
  auto* args   = static_cast<_output_args*>(state);
...
int vsnprintf(char* str, size_t len, const char* fmt, va_list ap) {
  ...
  int wlen    = printf_engine(&_vsnprintf_output, (void*)&args, fmt, ap);
  if (args.pos >= len) {
    str[len - 1] = '\0';
  } else {
    str[wlen] = '\0';
...
attr_noinline static char* longlong_to_string(char* buf, unsigned long long n, size_t len, uint flag, char* signchar) {
  size_t pos   = len;
  int negative = 0;
  ...
  while (n >= 10) {
    int digit = n % 10; // NOLINT(bugprone-narrowing-conversions,cppcoreguidelines-narrowing-conversions)
...
attr_noinline static char* longlong_to_hexstring(char* buf, unsigned long long u, size_t len, uint flag) {
  size_t pos        = len;
...
attr_noinline static size_t exponent_to_string(char* buf, int32_t exponent) {
  size_t pos = 0;
...
attr_noinline static char* double_to_string(char* buf, size_t len, double d, uint flag) {
  size_t pos          = 0;
  union double_int du = {d};

  uint32_t exponent   = (du.i >> 52) & 0x7ff;        // NOLINT(cppcoreguidelines-pro-type-union-access)
  uint64_t fraction   = (du.i & ((1ULL << 52) - 1)); // NOLINT(cppcoreguidelines-pro-type-union-access)
...
#define OUTREV(c)                                                                                                      \
  do {                                                                                                                 \
    if (&buf[pos] == buf)                                                                                              \
      ...
      size_t decimal_spot; // NOLINT(cppcoreguidelines-init-variables)
      decimal_spot = pos;

      /* print the integer portion */
      uint64_t u; // NOLINT(cppcoreguidelines-init-variables)
      if (exponent_signed >= 0) {
        ...
        char ch{0};
        char* s = longlong_to_string(buf, u, pos + 1, flag, &ch);
      ...
      uint32_t frac; // NOLINT(cppcoreguidelines-init-variables)
      // NOLINTNEXTLINE(bugprone-incorrect-roundings,bugprone-narrowing-conversions,cppcoreguidelines-narrowing-conversions)
      frac = ((d - u) * 1000000) + .5;
...
// NOLINTNEXTLINE(readability-function-cognitive-complexity)
attr_noinline static char* double_to_hexstring(char* buf, attr_unused size_t len, double d, uint flag) {
  size_t pos         = 0;
  union double_int u = {d};

  uint32_t exponent  = (u.i >> 52) & 0x7ff;        // NOLINT(cppcoreguidelines-pro-type-union-access)
  uint64_t fraction  = (u.i & ((1ULL << 52) - 1)); // NOLINT(cppcoreguidelines-pro-type-union-access)
  bool neg           = !!(u.i & (1ULL << 63));     // NOLINT(cppcoreguidelines-pro-type-union-access)
...
#define OUTPUT_CHAR(c)                                                                                                 \
  ...
  next_format:
    ...
    case '.':
      ...
      goto next_format;
    case '%':
      OUTPUT_CHAR('%'); // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
      break;
    case 'c':
      uc = va_arg(ap, unsigned int);
      OUTPUT_CHAR(uc); // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
      break;
    case 's':
    ...
    case 'x':
      ...
      if (flags & ALTFLAG) {
        OUTPUT_CHAR('0');                            // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
        OUTPUT_CHAR((flags & CAPSFLAG) ? 'X' : 'x'); // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
      }
      goto _output_string;
    case 'n':
      ptr = va_arg(ap, void*);
      if (flags & LONGLONGFLAG) {
        // NOLINTNEXTLINE(bugprone-narrowing-conversions,cppcoreguidelines-pro-type-cstyle-cast,cppcoreguidelines-narrowing-conversions)
    ...
    default:
      OUTPUT_CHAR('%'); // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
      OUTPUT_CHAR(c);   // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
      break;
    }
  ...
  _output_string:
    ...
    if (flags & LEFTFORMATFLAG) {
      ...
      if (flags & LEADZEROFLAG && signchar != '\0') {
        OUTPUT_CHAR(signchar); // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
      }

      /* pad according to the format string */
      for (; format_num > string_len; format_num--) {
        OUTPUT_CHAR(flags & LEADZEROFLAG ? '0' : ' '); // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
      }
