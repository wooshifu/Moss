...
.section .text.boot
    ...
    mov     tmp, #(0b11<<20)
    msr     cpacr_el1, tmp // FPEN bits[21:20] 0b11 This control does not cause execution of any instructions to be trapped.
    ...
    // cpuid 为 0 时才会执行以下代码直到 .Lmmu_enable_secondary
    ...
    mov     tmp, #0
.Lclear_top_page_table_loop: // equivalent to for(int i=0;i<MMU_KERNEL_PAGE_TABLE_ENTRIES_TOP;i++){ arm64_kernel_translation_table[i]=0; }
...
/* Read entry of mmu_initial_mappings (likely defined in platform.c) */
    ...
    ldp     size, tmp, [mmu_initial_mapping, %[__MMU_INITIAL_MAPPING_SIZE_OFFSET]] //tmp is mmu_initial_mapping.flags
    // equivalent to [paddr,vaddr,size,tmp]=[mmu_initial_mapping.phys,mmu_initial_mapping.virt,mmu_initial_mapping.size,mmu_initial_mapping.flags]

    tbzmask tmp, %[MMU_INITIAL_MAPPING_FLAG_DYNAMIC], .Lnot_dynamic
    // if it's dynamic, entry has to be patched up by platform_reset
    adr     paddr, _start
...
.Lmem_type_done:
    ...
    eor     vaddr, vaddr, #0xffff000000000000 // todo: (~0 << %[MMU_KERNEL_SIZE_SHIFT])#

    /* Check that top bits were all set */
    tst     vaddr, #0xffff000000000000 //todo: #(~0 << %[MMU_KERNEL_SIZE_SHIFT])
...
.Lphys_offset:
    ...
    calloc_bootmem_aligned new_page_table, tmp, tmp2, %[MMU_KERNEL_PAGE_SIZE_SHIFT], 1 // todo: 确认phys_offset的值为多少
...
.Lmap_range_has_page_table:
    ...
    and     x13, x14, #0xfffffffffffffffc // todo: 上一行被编译器优化为了这一行
    mov     tmp, #~0
    lsl     tmp, tmp, idx_shift
    bic     tmp, vaddr, tmp
...
.Lclear_tt_trampoline:
    ...
    add     tmp, tmp, #1
    cmp     tmp, #%[MMU_PAGE_TABLE_ENTRIES_IDENT] // 被优化为了 cmp     x9, #2, lsl #12 // =8192
...
LOCAL_DATA arm64_boot_args
    .skip (4 * 8) // 32 字节，用于存储x0,x1,x2,x3寄存器的值，因此为 32 字节
END_DATA arm64_boot_args
DATA arm64_boot_el
    .skip 8 // 8字节，用于存储 boot_el(即x28寄存器) 的值，因此为 8 字节
...
.align 3 + 9 // todo: 确认MMU_PAGE_TABLE_ENTRIES_IDENT_SHIFT的值为多少
DATA tt_trampoline
...
END_DATA tt_trampoline
      ...
      :
      : [SMP_CPU_ID_BITS] "i"(24), [MMU_KERNEL_PAGE_TABLE_ENTRIES_TOP] "i"(512),
        [__MMU_INITIAL_MAPPING_PHYS_OFFSET] "i"(0), [__MMU_INITIAL_MAPPING_SIZE_OFFSET] "i"(16),
        [MMU_INITIAL_MAPPING_FLAG_DYNAMIC] "i"(8), [MMU_INITIAL_MAPPING_FLAG_UNCACHED] "i"(2),
        [MMU_INITIAL_MAP_STRONGLY_ORDERED] "i"(0xffff0000000802f8UL), [MMU_INITIAL_MAPPING_FLAG_DEVICE] "i"(4),
        [MMU_PTE_ATTR_PXN] "i"(0x20000000000000), [MMU_KERNEL_PAGE_SIZE_SHIFT] "i"(12),
        [MMU_KERNEL_SIZE_SHIFT] "i"(0xffff'0000'0000'0000UL), [MMU_PTE_DESCRIPTOR_BLOCK_MAX_SHIFT] "i"(30),
        [MMU_PTE_L3_DESCRIPTOR_PAGE] "i"(3), [MMU_PTE_L012_DESCRIPTOR_BLOCK] "i"(1),
        [__MMU_INITIAL_MAPPING_SIZE] "i"(40), [MMU_PTE_L012_DESCRIPTOR_TABLE] "i"(3), [MMU_PTE_DESCRIPTOR_MASK] "i"(3),
        [MMU_PAGE_TABLE_ENTRIES_IDENT] "i"(8192), [MMU_IDENT_TOP_SHIFT] "i"(29), [SMP_CPU_CLUSTER_SHIFT] "i"(8),
        [SMP_MAX_CPUS] "i"(4), [MMU_KERNEL_TOP_SHIFT] "i"(39), [ARCH_DEFAULT_STACK_SIZE] "i"(4096),
        [MMU_INITIAL_MAP_DEVICE] "i"(0xffff000000080338), [MMU_PTE_KERNEL_DATA_FLAGS] "i"(0xffff000000080340),
        [MMU_PTE_KERNEL_RO_FLAGS] "i"(0xffff000000080350), [MMU_PTE_IDENT_FLAGS] "i"(0xffff000000080378),
        [MMU_MAIR_VAL] "i"(0xffff000000080380), [MMU_TCR_FLAGS_IDENT] "i"(0xffff000000080388),
        [MMU_TCR_FLAGS_KERNEL] "i"(0xffff000000080398)
