...
consteval auto MMU_KERNEL_TOP_SHIFT() {
  if (MMU_KERNEL_SIZE_SHIFT > MMU_LX_X(MMU_KERNEL_PAGE_SIZE_SHIFT, 0))
    return MMU_LX_X(MMU_KERNEL_PAGE_SIZE_SHIFT, 0);

  if (MMU_KERNEL_SIZE_SHIFT > MMU_LX_X(MMU_KERNEL_PAGE_SIZE_SHIFT, 1))
    return MMU_LX_X(MMU_KERNEL_PAGE_SIZE_SHIFT, 1);

  if (MMU_KERNEL_SIZE_SHIFT > MMU_LX_X(MMU_KERNEL_PAGE_SIZE_SHIFT, 2))
    return MMU_LX_X(MMU_KERNEL_PAGE_SIZE_SHIFT, 2);

  if (MMU_KERNEL_SIZE_SHIFT > MMU_LX_X(MMU_KERNEL_PAGE_SIZE_SHIFT, 3))
    return MMU_LX_X(MMU_KERNEL_PAGE_SIZE_SHIFT, 3);
}
...
consteval auto _MMU_IDENT_TOP_SHIFT() {
  if (MMU_IDENT_SIZE_SHIFT > MMU_LX_X(MMU_IDENT_PAGE_SIZE_SHIFT(), 0))
    return MMU_LX_X(MMU_IDENT_PAGE_SIZE_SHIFT(), 0);

  if (MMU_IDENT_SIZE_SHIFT > MMU_LX_X(MMU_IDENT_PAGE_SIZE_SHIFT(), 1))
    return MMU_LX_X(MMU_IDENT_PAGE_SIZE_SHIFT(), 1);

  if (MMU_IDENT_SIZE_SHIFT > MMU_LX_X(MMU_IDENT_PAGE_SIZE_SHIFT(), 2))
    return MMU_LX_X(MMU_IDENT_PAGE_SIZE_SHIFT(), 2);

  if (MMU_IDENT_SIZE_SHIFT > MMU_LX_X(MMU_IDENT_PAGE_SIZE_SHIFT(), 3))
    return MMU_LX_X(MMU_IDENT_PAGE_SIZE_SHIFT(), 3);
}
...
consteval auto MMU_PTE_IDENT_DESCRIPTOR() {
  if (MMU_IDENT_SIZE_SHIFT > MMU_LX_X(MMU_IDENT_PAGE_SIZE_SHIFT(), 2))
    return MMU_PTE_L012_DESCRIPTOR_BLOCK;
